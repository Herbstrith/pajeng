...
*/
#ifndef __PajeRastroSimulator_H__
#define __PajeRastroSimulator_H__
#include <map>
#include <vector>
#include <sstream>
#include <string>
...
class PajeRastroSimulator : public PajeSimulator {
private:
...
public:
  PajeRastroSimulator();
  PajeRastroSimulator(double stopat);
  PajeRastroSimulator(double stopat, int ignoreIncompleteLinks);
  PajeRastroSimulator (bool setRastroEvent);
  ~PajeRastroSimulator();
  void report (void);
...
private:
  void (PajeRastroSimulator::*invocation[PajeEventIdCount])(PajeTraceEvent*);
